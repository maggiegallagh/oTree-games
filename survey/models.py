...
class Player(BasePlayer):
    genderSlider = models.IntegerField(widget=widgets.Slider, min=-10, max=10, initial=0, label='Tell us whether you think men or women, on average, will perform better on this specific task. The bigger the gender difference, the more you should move the slider in that direction. Indicate your answer on the scale below, where 0 means no gender difference, -10 means "Women perform better on this task", and 10 means "Men perform better on this task". * ')
    ...
    gender = models.StringField(widget=widgets.RadioSelect, label="What is your gender? * ", choices =['Male','Female','None of the above','Prefer not to answer'])
    ...
    degree = models.StringField(widget=widgets.RadioSelect, label='What is the highest degree of level of school that you have completed? * ', choices=['Some college', "Associate's Degree (for example: AA, AS)", "Bachelor's Degree (for example: BA, BS)", "Master's Degree (for example: MA, MS, Meng, Med, MSW, MBA)", "Professional Degree beyond a Bachelor's Degree (for example: MD, DDS, DVM, LLB, JD)", "Doctoral Degree (for example: PhD, EdD)", "None of the above"])
    ...
    major = models.StringField(label='If you attended college, what was your primary major?', blank=True)
